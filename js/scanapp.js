	...
	$.Shop.prototype = {
		...
		photoShoot: function () {
			$('#cam-scan').on('click', function () {
				$(this).addClass('photo-taken')
					...
					.find('.prod-process').text('Redirigiendo al producto');
		...
		updateTotal: function () {
			...
			if (items.length == 0) {
				...
				$('.nav-bar .badge').html('0').addClass('d-none').parent('a').attr('aria-label', '0 productos en carrito');
			} else {
				...
				var badge = $('.nav-bar .badge');
				badge.html(totalQty).removeClass('d-none');
				if (totalQty == 1) {
					badge.parent('a').attr('aria-label', 'Un producto en carrito');
				} else {
					badge.parent('a').attr('aria-label', totalQty + ' productos en carrito');
				}
			}
		},
		...
		showNotifications: function () {
			...
			if (not == null) {
				...
				not = self.storage.getItem('notifications');
			}
			if (not == 0) {
				$('.notifications').attr('aria-label', 'Sin Notificaciones').find('.badge').remove();
				$('#not1').html('No hay notificaciones sin leer');
			} else {
		...
		_emptyCart: function () {
			this.storage.removeItem(self.cartName);
		},
